...
export default function AdminPage() {
  ...
  async function syncAll() {
    ...
    for (let i = 0; i < unprocessed.length; i++) {
      const filePath = unprocessed[i]
      setSyncProgress({ done: i, total: unprocessed.length, current: filePath })

      await fetch(`/api/admin/upload?key=${key}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    setSyncProgress(p => ({ ...p, done: p.total, current: '' }))
    setSyncing(false)
    loadDocuments()
  }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {bucketTotal} files in bucket · {documents.length} processed · {unprocessed.length} pending
            </p>
          </div>
        ...
        {activeTab === 'sync' && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Extracts text from all unprocessed PDFs in the storage bucket and saves them to the database.
            </p>
              ...
              <>
                ...
                {syncing && (
                  <div className="mb-4">
                    <div className="flex justify-between text-sm text-gray-600 mb-1">
                      <span>Processing {syncProgress.current}</span>
                ...
                >
                  {syncing
                    ...
                    : `Process ${unprocessed.length} files`}
                </button>
              </>
            )}
          </div>
