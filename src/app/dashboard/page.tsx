...
export default function DashboardPage() {
  ...
  const weakSections = sectionPerf.filter(s => s.pct < 70).sort((a, b) => a.pct - b.pct).slice(0, 5)
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 24 }}>
              ...
              <div style={{ backgroundColor: 'white', borderRadius: 16, border: '1px solid #E2E8F0', padding: '24px' }}>
                <div style={{ marginBottom: 16 }}>
                  <h2 style={{ fontSize: 14, fontWeight: 700, color: '#0B1F33', marginBottom: 4 }}>Skills Map</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {weakSections.length > 0 && (
                  <div style={{ backgroundColor: 'white', borderRadius: 16, border: '1px solid #E2E8F0', padding: '24px', flex: 1 }}>
                ...
                {strongSections.length > 0 && (
                  <div style={{ backgroundColor: 'white', borderRadius: 16, border: '1px solid #E2E8F0', padding: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                      ...
                      <h2 style={{ fontSize: 14, fontWeight: 700, color: '#0B1F33' }}>Strong areas</h2>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {strongSections.map(s => (
                        <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: 13, color: '#64748B' }}>{s.title}</span>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: 16, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
              <div style={{ padding: '20px 28px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <h2 style={{ fontSize: 14, fontWeight: 700, color: '#0B1F33' }}>Exam History</h2>
                {/* Mini trend chart */}
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 32 }}>
                  {completedAttempts.slice().reverse().map(a => {
                    const pct = Math.round((a.score || 0) / (a.total_questions || 105) * 100)
                    return (
                      <div key={a.id} style={{ width: 12, borderRadius: 2, height: `${Math.max(pct / 100 * 32, 4)}px`, backgroundColor: pct >= 70 ? '#16A34A' : pct >= 50 ? '#D97706' : '#DC2626', opacity: 0.7 }} />
                    )
                  })}
                </div>
              </div>
