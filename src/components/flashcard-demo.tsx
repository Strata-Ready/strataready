    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => { setCardIndex(i => Math.min(DEMO_CARDS.length - 1, i + 1)); setFlipped(false) }} disabled={cardIndex === DEMO_CARDS.length - 1}
      ...
      <p style={{ fontSize: 12, color: '#94A3B8', marginTop: 12, textAlign: 'center' }}>
        Card {cardIndex + 1} of {DEMO_CARDS.length} · <Link href="/flashcards" style={{ color: '#00a79d', textDecoration: 'none', fontWeight: 600 }}>See all sections →</Link>
      </p>
    </div>
  )
}
